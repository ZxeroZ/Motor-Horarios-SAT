...
export default function ScheduleAnalysis() {
    ...
    return (
      <div className="analysis-container">
        <div className="analysis-loading">
          ...
          <p style={{color: 'var(--text-muted)'}}>Cargando analisis del horario...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="analysis-container">
        <div className="analysis-error">
          ...
          <button onClick={loadData} className="btn-save" style={{marginTop: '1rem'}}>Reintentar</button>
        </div>
      </div>
    );
  }
  ...
  const estadoLabel = {
    'OPTIMAL': 'una solucion optima',
    'FEASIBLE': 'una solucion valida',
    'INFEASIBLE': 'un problema (sin solucion)',
  ...
  if (problemas_detectados && problemas_detectados.length > 0) {
    problemas_detectados.forEach((p) => {
      const match = p.match(/^(.+?): (\d+)h en (\w+) \(promedio ([\d.]+)h\/d[ií]a\)/);
      if (match) {
        ...
        if (horasNum >= 7) {
          severidad = 'urgente';
          accion = `Distribuir al menos ${Math.ceil(exceso / 2)} horas en otro dia para evitar cansancio y mejorar la calidad de ensenanza.`;
        } else {
          accion = `Considerar mover ${exceso > 2 ? 2 : 1} hora(s) a otro dia con menos carga.`;
        }
        problemasDirector.push({
          ...
          severidad,
          accion,
          texto: `${nombre} tiene ${horasNum} horas de clase concentradas un solo dia (${dia}), cuando lo ideal serian maximo ${Math.round(promedioNum) + 2} horas. Esto puede generar cansancio y afectar la calidad de las clases.`
        });
      }
    });
  }
    ...
    <div className="analysis-container">
      ...
      <div className="executive-summary">
        ...
        <div className="exec-text">
          <p>
            ...
            El motor encontro {estadoLabel} en {tiempoStr}.
            {problemasDirector.length === 0
              ? ' No se detectaron problemas. El horario esta listo para usar.'
              : ` Se detectaron ${problemasDirector.length} punto${problemasDirector.length > 1 ? 's' : ''} de atencion.`
            }
          </p>
        </div>
      </div>
      ...
      <div className="stats-grid">
        <div className="stat-card">
          <span className="stat-icon" style={{fontSize: '1.5rem'}}>
            <span className="material-icons-outlined" style={{fontSize: '1.5rem', color: 'var(--accent)'}}>menu_book</span>
          </span>
        ...
        <div className="stat-card">
          <span className="stat-icon" style={{fontSize: '1.5rem'}}>
            <span className="material-icons-outlined" style={{fontSize: '1.5rem', color: 'var(--accent)'}}>people</span>
          </span>
        ...
        <div className="stat-card">
          <span className="stat-icon" style={{fontSize: '1.5rem'}}>
            <span className="material-icons-outlined" style={{fontSize: '1.5rem', color: 'var(--accent)'}}>business</span>
          </span>
        ...
        <div className={`stat-card ${estado === 'OPTIMAL' ? 'stat-success' : estado === 'INFEASIBLE' ? 'stat-danger' : 'stat-warning'}`}>
          <span className="stat-icon" style={{fontSize: '1.5rem'}}>
            <span className="material-icons-outlined" style={{fontSize: '1.5rem', color: estado === 'OPTIMAL' ? 'var(--success)' : estado === 'INFEASIBLE' ? 'var(--danger)' : 'var(--warning)'}}>
              {estado === 'OPTIMAL' ? 'check_circle' : estado === 'INFEASIBLE' ? 'cancel' : 'warning'}
            </span>
          </span>
      ...
      {problemasDirector.length > 0 && (
        <div className="problems-section">
          ...
          <p className="section-subtitle">Situaciones que podrian mejorarse para un mejor horario</p>
              ...
              <div key={idx} className={`problem-card problem-${p.severidad}`}>
                <div className="problem-header">
                  ...
                  <span className="problem-title">
                    {p.severidad === 'urgente' ? 'Sobrecarga' : 'Mejorable'} &mdash; {p.nombre}
                  </span>
                </div>
                ...
                <div className="problem-action">
                  ...
                  <strong>Accion sugerida:</strong> {p.accion}
                </div>
              </div>
            ))}
          </div>
      ...
      {diasActivos > 1 && (
        <div className="distribution-section">
          ...
          <p className="distribution-note">
            {maxDia[1] > minDia[1] * 2
              ? `${maxDia[0]} es el dia mas cargado (${maxDia[1]}h) y ${minDia[0]} tiene muy poca actividad (${minDia[1]}h). Podria considerar redistribuir.`
              : `${maxDia[0]} es el dia mas cargado con ${maxDia[1]} horas.`
            }
          </p>
        </div>
      )}
      ...
      {problemasDirector.length === 0 && (
        <div className="all-good-section">
          ...
          <h3>Todo listo!</h3>
          <p>El horario esta bien distribuido. No hay profesores con sobrecarga ni desequilibrios importantes.</p>
        </div>
      )}
    </div>
  );
