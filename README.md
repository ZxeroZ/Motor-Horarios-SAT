...
* **Exclusividad de Espacio Físico (Recursos Compartidos):** A través de la propiedad `"requiere_espacio_unico": true` en el JSON de entrada, el motor puede modelar cursos que dependen de instalaciones físicas irreemplazables (ej. un laboratorio de química, una cancha de fútbol). El CP-SAT impone la regla matemática `AddAtMostOne` agrupada por Sede, asegurando que jamás dos secciones de la misma sede utilicen el recurso en el mismo bloque de tiempo.
* **Reservas Disyuntivas de Bloques (Floating Blackouts):** Mediante el nodo `"bloques_reservados"`, el colegio puede establecer conjuntos de slots (opciones) prohibidos para dictado de clases (útil para simulacros, formaciones, etc.). Esta es una restricción disyuntiva inteligente: si se configuran múltiples caminos (ej. bloquear en la mañana `[1, 2, 3]` o después de recreo `[4, 5, 6]`), el motor matemático evaluará y *decidirá* de forma autónoma qué vía reservar para maximizar la cobertura general de la sede. Adicionalmente, esta reserva puede focalizarse usando el filtro `"grados_afectados"`.
